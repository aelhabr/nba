...
filepath_lines <- "data/bigfour-cleaned.csv"
...
#'
# Import. ----
results_cleaned <- readr::read_csv(filepath_import)
tms_geo <- readr::read_csv(filepath_locations)
lines <- readr::read_csv(filepath_lines)
...
results_lines <-
  results_base %>%
  left_join(lines %>% select(-pts_home, -pts_away)) %>%
  mutate(prob_home = ifelse(is.na(prob_home), mean(prob_home, na.rm = TRUE), prob_home)) %>%
  rename(prob_h2a = prob_home)
...
if (remove_tempvars == TRUE) {
  rm(list = c("results_cleaned", "lines", "tms_geo", "results_base"))
}
#'
...
#+ include = FALSE
if (export == TRUE) {
  readr::write_csv(results_cumcalcs_eoy, filepath_export_eoy)
}
...
#'
if (export == TRUE) {
  readr::write_csv(output, filepath_export)
  readr::write_csv(colnames_tidy, filepath_export_colnames)
}
#'
#'
#'
