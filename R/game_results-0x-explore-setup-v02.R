...
#'
#'
results_prepared_min %>% filter(tm == "ATL" & rn == 1)
results_prepared_min %>% filter(tm == "UTA" & rn == 1)
#'
#'
#'
# vars_factor <-
...
cut_4_tiers <- function(d, colname_bare) {
  colname_quo <- enquo(colname_bare)
  d %>%
    summarise(
      lmax = quantile(!!colname_quo, 0),
      lmid = quantile(!!colname_quo, 1 / 4),
      t = quantile(!!colname_quo, 1 / 2),
      wmid = quantile(!!colname_quo, 3 / 4),
      wmax = quantile(!!colname_quo, 1)
    ) %>%
    gather(tier, value) %>%
    arrange(value)
}

cut_6_tiers <- function(d, colname_bare) {
  colname_quo <- enquo(colname_bare)
  d %>%
    summarise(
      lmax = quantile(!!colname_quo, 0),
      lbig = quantile(!!colname_quo, 1 / 6),
      lclose = quantile(!!colname_quo, 2 / 6),
      t = quantile(!!colname_quo, 3 / 6),
      wclose = quantile(!!colname_quo, 4 / 6),
      wbig = quantile(!!colname_quo, 5 / 6),
      wmax = quantile(!!colname_quo, 1)
    ) %>%
    gather(tier, value) %>%
    arrange(value)
...
# This is another highly-specific function.
cut_results <- function(vals_cont, vals_disc, labs) {
  cut(vals_cont, vals_disc, labs[-1], include.lowest = TRUE)
}
...
results_prepared_min <-
  ...
  mutate(
    result_tier6 = cut_results(
      ...
      results_tiers6_labels$label
    )
  ) %>%
  mutate(
    prob_tier4 = cut_results(
      results_prepared_min$prob,
      probs_tiers4_labels$value,
      probs_tiers4_labels$label
    )
  ) %>%
  mutate(
    prob_tier6 = cut_results(
      results_prepared_min$prob,
      probs_tiers6_labels$value,
      probs_tiers6_labels$label
    )
  )
...
results_prepared_min <-
  results_prepared_min %>%
  select(rn, result, result_tier4, result_tier6, prob, prob_tier4, prob_tier6, everything()) %>%
  mutate_at(vars(starts_with("result|prob")), funs(as.factor(str_to_lower(.))))
results_prepared_min
#'
#'
#'
...
#'
if(remove_tempvars == TRUE) {
  rm(list = ls(pattern = "tiers|labels"))
}
#'
...
if (try_old_code == TRUE) {
  ...
  lcombo_info
  lcombo_info$remove
  for (rn in 1:length(lcombo_info$linearCombos)) {
    print(names(results_eoy_stats)[lcombo_info$linearCombos[[rn]]])
    print(names(results_eoy_stats)[lcombo_info$remove[[rn]]])
  }
...
#'
# Use results_eoy here so as to not extract unwanted "variations" (rn.e. _td, _1g, etc.) of each variable.
vars_ff <-
  names(results_eoy) %>%
...
vars_ctg <-
  names(results_eoy) %>%
  str_subset("ortg|efg|tov|orb|ftr")
vars_ctg
...
colnames_base_join <- c("date", "season", "tm_home", "tm", "tm_opp")
vars_ctg_td <- vars_ctg %>% str_c(., "_td")
vars_ctg_td_opp <- vars_ctg_td %>% str_c(., "_opp")

results_prepared_min <-
  results_prepared_min %>%
  # select(one_of(c("rn", vars_y, vars_base))) %>%
  inner_join(results_prepared %>% select(one_of(
    c(colnames_base_join, vars_ctg_td, vars_ctg_td_opp)
  ))) %>%
...
session::save.session(file = "R/RData/explore-setup.RData")
#'
#'
#'
