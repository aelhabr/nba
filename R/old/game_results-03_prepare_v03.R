...
results_side1 <-
  results_renamed %>%
  rename_at(vars(contains("_home_1g")), funs(str_replace_all(., "_home_", "_a_"))) %>%
  rename_at(vars(contains("_away_1g")), funs(str_replace_all(., "_away_", "_b_"))) %>%
  mutate(
    tm_a = tm_home,
    tm_b = tm_away,
    pts_a = pts_home,
...
results_side1 %>% group_by(season) %>% count()

results_side2 <-
  results_renamed %>%
  rename_at(vars(contains("_home_1g")), funs(str_replace_all(., "_home_", "_b_"))) %>%
  rename_at(vars(contains("_away_1g")), funs(str_replace_all(., "_away_", "_a_"))) %>%
  mutate(
    tm_a = tm_away,
    tm_b = tm_home,
    pts_a = pts_away,
...
results_side2 %>% group_by(season) %>% count()
...
results_2x <-
  ...
  arrange(date, season, tm_home, tm_away, tm_a)
...
#'
#' In prior versions of thsis script, specific stats columns were filtered out
#' in order to reduce the size of the data set. Now, all stats are included.
#' (This is why there is a new variable created for no apparent reason.)
#'
  ...
  mutate(dist = ifelse(rn == 1, 0,
                       ifelse(
                         ...
                         ifelse(tm_a == tm_home,
                                0,
                                calc_dist(
                                  lag(lng_a_1g), lag(lat_a_1g), lng_b_1g, lat_b_1g
                                )),
                         ifelse(tm_a == tm_home,
                                calc_dist(
                                  lag(lng_b_1g), lag(lat_b_1g), lng_a_1g, lat_a_1g
                                ),
                                0)
                       ))) %>%
  select(-rn) %>%
...
results_with_dist <-
  results_with_dist %>%
  select(-lng_a_1g,-lat_a_1g,-lng_b_1g,-lat_b_1g)
...
colnames_ab_1g <-
  ...
  str_subset("_(a|b)_1g$") %>%
  str_subset("^(?!tm_).*")

results_cumcalcs_temp <-
  results_cumcalcs %>%
  group_by(season, tm_a) %>%
  mutate_at(vars(colnames_ab_1g), funs(td = cummean)) %>%
  ungroup() %>%
  rename_at(vars(ends_with("_a_1g_td")), funs(str_replace_all(., "_a_1g_td", "_a_td"))) %>%
  rename_at(vars(ends_with("_b_1g_td")), funs(str_replace_all(., "_b_1g_td", "_b_td")))
...
results_cumcalcs_renamed <-
  results_cumcalcs %>%
  rename_at(vars(colnames_calc_manual), funs(str_c(., "_a"))) %>%
  rename_at(vars(ends_with("td_a")), funs(str_replace_all(., "td_a$", "_a_td"))) %>%
  rename_at(vars(ends_with("_a_td")), funs(str_replace_all(., "_a_td$", "_off_td"))) %>%
  rename_at(vars(ends_with("_b_td")), funs(str_replace_all(., "_b_td$", "_def_td")))
#'
...
colnames_calc_manual_off_td <-
  colnames_calc_manual %>%
  str_c("_off") %>%
  str_replace_all("td_off$", "_off_td") %>%
  str_subset("_off_td$")
colnames_calc_manual_off_td
...
results_cumcalcs_eoy <-
  ...
  select(one_of(c("season", "tm_a", colnames_td))) %>%
  ungroup() %>%
  rename(tm = tm_a) %>%
...
#'
colnames_join <- c("date", "tm_home", "tm_away", "tm_a")
colnames_vars_join <-
  names(results_cumcalcs_renamed) %>%
...
results_cumcalcs_2sided <-
  ...
  inner_join(
    ...
    by = c("date" = "date", "tm_home", "tm_away", "tm_b" = "tm_a")
  )
