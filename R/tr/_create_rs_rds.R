...
#         d_stat_join <- d_stat
#       } else {
#         d_stat_join <- inner_join(d_stat_join, d_stat, by = c("team", "yr_start"))
#       }
#     }
#     if (i == 1) {
#       tbl_yrs_rbind <- d_stat_join
#     } else {
...
import_and_append <- function(yrs, stats) {
    ...
    while (i <= length(yrs)) {
        ...
        while (j <= length(stats)) {
            ...
            ws_name <- substr(paste0("d_", stat), 1, 31)
            d_stat_raw <- get_tbl_from_xlsx(ws_name = ws_name, 
                                              filter_date_input = filter_date, 
                                              save_dir = dir_data, 
                                              save_file_name_w_ext = wb_name_w_ext)
            
            d_stat <- d_stat_raw %>% arrange(team) %>% dplyr::select(one_of(stat))
            
            m_stats[, j + 2] <- as.matrix(d_stat, dimnames = NULL)
            j <- j + 1
        }
...
rds_raw_original <- rds_raw
rds_raw$team <- sort(g_teams$team)
...
file_name_w_ext <- "d_scraped_stats_classification_raw.rds"
