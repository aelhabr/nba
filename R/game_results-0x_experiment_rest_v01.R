...
if (run_unused_code == TRUE) {
  ...
  coefs_explpct_baseline <- calc_explpct(fmla_ff_1g, data_tst)
  coefs_explpct_1a_2a <-
...
#'
#'
# Ingore the warninga bout the intercept here.
# The values are nearly identifical when an intercept term is added.
car::vif(lm(fmla_rest_1, data_trn))
relimp_1 <- relaimpo::calc.relimp(lm(fmla_rest_1_intercept, data_tst), type = "lmg", rela = TRUE)
