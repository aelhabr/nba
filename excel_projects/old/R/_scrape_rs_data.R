...
# Variables to be used. #### TODO: Then integrate this into the get_tbl_stat() set of functions.
...
scrape_rs_stats <- function(yrs, stats, links) {
    ...
    while (i <= length(yrs)) {
        ...
        while (j <= length(stats)) {
            ...
            tbl_stat_raw <- get_tbl_stat_from_html(html_file)
            
            unlink(html_file, recursive = TRUE)
            tbl_stat <- process_tbl_stat(tbl_stat_raw = tbl_stat_raw, stat = stat, filter_date = filter_date)
            ws_name <- substr(paste0("d_", stat), 1, 31)
            tbl_stat_saved <- save_html_to_xlsx(tbl_stat = tbl_stat, filter_date_input = filter_date, ws_name = ws_name, save_dir = dir_data, 
